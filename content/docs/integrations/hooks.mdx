---
title: Hooks
description: Configure hooks to customise the HTTP request/response lifecycle in Authula.
---
...
adminOnlyHook := models.Hook{
    ...
    Matcher: func(reqCtx *models.RequestContext) bool {
        ...
        return strings.HasPrefix(reqCtx.Path, "/admin")
    },
    Handler: func(reqCtx *models.RequestContext) error {
        if reqCtx.Actor == nil || reqCtx.Actor.Type != models.ActorUser {
            reqCtx.SetJSONResponse(http.StatusUnauthorized, map[string]any{"message": "Authentication required"})
            reqCtx.Handled = true
            return nil
        }
        ...
        isAdmin, err := hasRole(userID, "admin")
        if err != nil || !isAdmin {
            reqCtx.SetJSONResponse(http.StatusForbidden, map[string]any{"message": "Admin access required"})
            reqCtx.Handled = true
            return nil
        }
...
setUserDataHook := models.Hook{
    ...
    Handler: func(reqCtx *models.RequestContext) error {
        ...
        if userID != "" {
            ...
            reqCtx.Values["user_profile"] = userProfile
        }
...
auditLogHook := models.Hook{
    ...
    Handler: func(reqCtx *models.RequestContext) error {
        if profile, exists := reqCtx.Values["user_profile"]; exists {
            // Log the action with user information
            logAuditEvent(profile.(UserProfile), reqCtx.Path, reqCtx.Method)
        }
        return nil
    },
}
...
analyticsHook := models.Hook{
    ...
    Handler: func(reqCtx *models.RequestContext) error {
        if reqCtx.Actor == nil || reqCtx.Actor.Type != models.ActorUser {
            return nil // No user, skip analytics
        }
...
rateLimitHook := models.Hook{
    ...
    Handler: func(reqCtx *models.RequestContext) error {
        ...
        if err := checkRateLimit(reqCtx.ClientIP); err != nil {
            reqCtx.SetJSONResponse(http.StatusTooManyRequests, map[string]any{"message": "Rate limit exceeded"})
            reqCtx.Handled = true
            return nil
        }
        return nil
...
- Use `reqCtx.Values` to share data between hooks
